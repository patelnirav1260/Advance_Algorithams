...
class MinCut {
  int numberOfVerticies, verticiesLeft, count, i, j, k, l, vertexV, vertexU,
      currentMincut, mincutAns;
 ...
 public:
    ...
    while (verticiesLeft > 2) {
      ...
      for (i = 0; i < verticiesLeft; i++) {
        for (j = i + 1; j < verticiesLeft; j++) {
          if (initialGraph[i][j]) edges.push_back({i, j});
        }
      }
      ...
      currentGraph = rough;
      vector<vector<int>> rough1(verticiesLeft - 1,
                                 vector<int>(verticiesLeft - 1, 0));
  ...
  void copyGraph() {
    for (i = 0, k = 0; i < verticiesLeft; i++) {
      if (i == vertexV) continue;
      for (j = 0, l = 0; j < verticiesLeft; j++) {
        if (j == vertexV) continue;
